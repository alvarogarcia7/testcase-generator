...
# Get the script directory and source shared libraries
...
# Validate arguments
if [[ $# -eq 0 ]]; then
    log_error "Missing required argument: YAML file path(s)"
...
if [[ ! -f "$SCHEMA_FILE" ]]; then
    ...
    log_error "Set SCHEMA_FILE environment variable to specify the schema"
    exit 1
fi
