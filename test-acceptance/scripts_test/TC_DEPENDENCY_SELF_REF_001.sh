...
# system: Self-reference detection is enabled
# system: Bash shell is available
# system: Self-reference detection is enabled

# Initial Conditions
# system: This test includes itself - should be detected as circular dependency
# system: TC_DEPENDENCY_SELF_REF_001 -> TC_DEPENDENCY_SELF_REF_001 (SELF REFERENCE!)
# filesystem: Temporary directory /tmp is writable

# Test Sequence 1: Self Reference Test
# Test that references itself in dependencies
# Sequence Initial Conditions
# system: Self-reference should be detected immediately

# Step 1: Attempt to execute with self-reference
LOG_FILE="TC_DEPENDENCY_SELF_REF_001_sequence-1_step-1.actual.log"
COMMAND_OUTPUT=""
set +e
COMMAND_OUTPUT=$({ echo 'This should not execute due to self-reference'; } 2>&1 | tee "$LOG_FILE")
EXIT_CODE=$?
set -e
...
if grep -q 'Self-reference\|Circular dependency' <<< "$COMMAND_OUTPUT" || true; then
    VERIFICATION_OUTPUT_PASS=true
fi

if [ "$VERIFICATION_RESULT_PASS" = true ] && [ "$VERIFICATION_OUTPUT_PASS" = true ]; then
    echo "[PASS] Step 1: Attempt to execute with self-reference"
else
    echo "[FAIL] Step 1: Attempt to execute with self-reference"
    echo "  Command: echo 'This should not execute due to self-reference'"
...
# Test Sequence 2: Sequence Self Reference
# Test sequence that references itself
# Sequence Initial Conditions
# system: Sequence-level self-reference should also be detected
# system: Sequence-level self-reference should also be detected

# Step 1: Attempt to execute with sequence self-reference
LOG_FILE="TC_DEPENDENCY_SELF_REF_001_sequence-2_step-1.actual.log"
COMMAND_OUTPUT=""
set +e
COMMAND_OUTPUT=$({ echo 'This should not execute due to sequence self-reference'; } 2>&1 | tee "$LOG_FILE")
EXIT_CODE=$?
set -e
...
if [ "$VERIFICATION_RESULT_PASS" = true ] && [ "$VERIFICATION_OUTPUT_PASS" = true ]; then
    echo "[PASS] Step 1: Attempt to execute with sequence self-reference"
else
    echo "[FAIL] Step 1: Attempt to execute with sequence self-reference"
    echo "  Command: echo 'This should not execute due to sequence self-reference'"
    echo "  Exit code: $EXIT_CODE"
    echo "  Output: $COMMAND_OUTPUT"
    echo "  Result verification: $VERIFICATION_RESULT_PASS"
    echo "  Output verification: $VERIFICATION_OUTPUT_PASS"
