...
# Test Case: TC_DEPENDENCY_CIRCULAR_001
# Description: Circular dependency detection test - demonstrates detection of circular dependency errors (A includes B, B includes A)

JSON_LOG="TC_DEPENDENCY_CIRCULAR_001_execution_log.json"
...
# system: This test creates circular dependency
# system: TC_DEPENDENCY_CIRCULAR_001 -> TC_DEPENDENCY_CIRCULAR_002 -> TC_DEPENDENCY_CIRCULAR_001 (CIRCULAR!)

# Test Sequence 1: Circular Dependency Test
# This test has a circular dependency with TC_DEPENDENCY_CIRCULAR_002
# Sequence Initial Conditions
# system: Circular dependency should be detected during validation

# Step 1: Attempt to execute with circular dependency
LOG_FILE="TC_DEPENDENCY_CIRCULAR_001_sequence-1_step-1.actual.log"
COMMAND_OUTPUT=""
set +e
...
# Step 2: Verify circular dependency error
LOG_FILE="TC_DEPENDENCY_CIRCULAR_001_sequence-1_step-2.actual.log"
COMMAND_OUTPUT=""
set +e
COMMAND_OUTPUT=$({ echo 'Circular dependency error should have been raised'; } 2>&1 | tee "$LOG_FILE")
EXIT_CODE=$?
set -e
...
if [ "$VERIFICATION_RESULT_PASS" = true ] && [ "$VERIFICATION_OUTPUT_PASS" = true ]; then
    echo "[PASS] Step 2: Verify circular dependency error"
else
    echo "[FAIL] Step 2: Verify circular dependency error"
    echo "  Command: echo 'Circular dependency error should have been raised'"
    echo "  Exit code: $EXIT_CODE"
    echo "  Output: $COMMAND_OUTPUT"
    echo "  Result verification: $VERIFICATION_RESULT_PASS"
    echo "  Output verification: $VERIFICATION_OUTPUT_PASS"
