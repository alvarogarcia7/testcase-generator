use crate::TestCaseStorage;
use anyhow::Result;
use std::collections::{HashMap, HashSet};
use std::path::Path;
use testcase_models::{Expected, InitialConditionItem, Step, TestSequence};
            ...
            for sequence in &test_case.test_sequences {
                ...
                for (device_name, conditions) in &sequence.initial_conditions.devices {
                    let string_conditions: Vec<String> = conditions
                        .iter()
                        .map(|item| match item {
                            InitialConditionItem::String(s) => s.clone(),
                            InitialConditionItem::RefItem { reference } => {
                                format!("ref: {}", reference)
                            }
                            InitialConditionItem::TestSequenceRef { test_sequence } => format!(
                                "test_sequence: id={}, step={}",
                                test_sequence.id, test_sequence.step
                            ),
                        })
