use crate::editor::{EditorFlow, TestCaseEditor};
...
"#;
    ...
    pub fn prompt_verification_with_templates(
        ...
        use crate::fuzzy::TestCaseFuzzyFinder;
        use testcase_verification::VerificationTemplateLibrary;
                ...
                Some(selected_display) => {
                    if let Some(template) = library.get_template_by_display_name(&selected_display)
                    {
                        ...
                        if use_as_is {
                            ...
                            return Ok(testcase_models::Verification {
                                result: testcase_models::VerificationExpression::Simple(result),
                                output: testcase_models::VerificationExpression::Simple(output),
                                output_file: None,
                                general: None,
                            });
                        }
        ...
        Ok(testcase_models::Verification {
            result: testcase_models::VerificationExpression::Simple(result),
            output: testcase_models::VerificationExpression::Simple(output),
            output_file: None,
            general: None,
    ...
    pub fn prompt_capture_vars(
        oracle: &Arc<dyn Oracle>,
    ) -> Result<Option<testcase_models::CaptureVarsFormat>> {
        use testcase_models::{CaptureVar, CaptureVarsFormat};
    ...
    pub fn prompt_general_verifications(
        oracle: &Arc<dyn Oracle>,
    ) -> Result<Option<Vec<testcase_models::GeneralVerification>>> {
    ...
    fn apply_selected_template(
        template: &testcase_verification::VerificationTemplate,
        oracle: &Arc<dyn Oracle>,
    ) -> Result<testcase_models::Verification> {
        println!("\n✓ Selected template: {}", template.name);
        println!("  Description: {}", template.description);
