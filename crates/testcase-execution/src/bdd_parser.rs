...
//                return Err("Pattern with optional non-capturing groups and multiple parameters is not supported. Optional groups can cause parameter misalignment.".into());
        ...
        if param_names.len() > 1 {
            ...
            // We need to detect patterns like (?:...)?  where ... contains (...)
            // This requires checking for nested structures
            let mut i = 0;
            ...
            while i < chars.len() {
                ...
                if i + 2 < chars.len() && chars[i] == '(' && chars[i+1] == '?' && chars[i+2] == ':' {
                    ...
                    if depth == 0 && j < chars.len() && chars[j] == '?' && has_capture {
                        return Err(
                            "Pattern contains optional groups with capture groups, which is incompatible with multiple parameter conversion"
                                .into(),
                        );
                    }
                }
